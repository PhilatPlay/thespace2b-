...
Follow.prototype.validate = async function(action) {
  ...
  if (action == 'create') {
    if (doesFollowAlreadyExist) {
      this.errors.push(`You are already following ${req.params.username}.`);
    }
  }
  if (action == 'delete') {
    if (!doesFollowAlreadyExist) {
      this.errors.push(
        `You are not following ${req.params.username}; you can not stop following someone you dont already follow.`
      );
    }
  }
  // should not be able to follow yourself
  if (this.followedId.equals(this.authorId)) {
    this.errors.push('You are already following yourself.');
  }
};
